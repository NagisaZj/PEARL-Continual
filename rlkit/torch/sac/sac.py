...
from rlkit.core.rl_algorithm import MetaRLAlgorithm,ExpAlgorithmFin2,MetaRLAlgorithmCont,MetaRLAlgorithmCont2
...
class PEARLSoftActorCriticCont(MetaRLAlgorithmCont):
    def __init__(
            self,
            envs,
            per_task_steps,
...
class PEARLSoftActorCriticCont2(MetaRLAlgorithmCont2):
    def __init__(
            self,
            envs,
            per_task_steps,
...
class PEARLSoftActorCriticContPi(MetaRLAlgorithmCont):
    def __init__(
            self,
            envs,
            per_task_steps,
    ...
    def _do_training(self, indices):
        ...
        # sample context batch
        context_batch = self.sample_context(indices)
        previous_task_batch =  self.sample_context(indices)
        ...
        for i in range(num_updates):
            context = context_batch[:, i * mb_size: i * mb_size + mb_size, :]
            previous_task_data = previous_task_batch[:, i * mb_size: i * mb_size + mb_size, :]

            previous_task_grad = self.compute_grad(indices, previous_task_data)
            self._take_step(indices, context)
    ...
    def _take_step(self, indices, context):
        ...
        # run inference in networks
        policy_outputs, task_z = self.agent.forward_2(obs, context)
        new_actions, policy_mean, policy_log_std, log_pi = policy_outputs[:4]
        ...
        # policy update
        # n.b. policy update includes dQ/da
        log_policy_target = self._min_q(obs, new_actions, task_z)
        ...
        policy_reg_loss = mean_reg_loss + std_reg_loss + pre_activation_reg_loss
        policy_loss = policy_loss + policy_reg_loss
        # print(torch.autograd.grad(policy_loss,self.agent.context_encoder.parameters()))
    ...
    def compute_grad(self, indices, context):
    ...
    def _take_step_gp(self, indices, context):
