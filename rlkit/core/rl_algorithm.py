...
class MetaRLAlgorithmCont(metaclass=abc.ABCMeta):
    def __init__(
            self,
            envs,
            agent,
        ...
        self.agent = agent
        self.exploration_agent = agent # Can potentially use a different policy purely for exploration rather than also solving tasks, currently not being used
        self.per_task_steps = per_task_steps
        self.meta_batch = meta_batch
        self.num_iterations = num_iterations
        self.num_train_steps_per_itr = num_train_steps_per_itr
        self.num_initial_steps = num_initial_steps
    ...
    def train(self):
        ...
        task_idx = 0
        it_ = 0
        while task_idx<len(self.envs):
            self.task_idx = task_idx
            self.task_idx_train = task_idx
            num_samples_this_task = 0

            while num_samples_this_task<self.per_task_steps:
                # if (self.per_task_steps-num_samples_this_task)==1000:
                #     self.enc_replay_buffer.task_buffers[task_idx].clear()
                #     self.replay_buffer.task_buffers[task_idx].clear()
                self.task_idx = self.task_idx_train
                self.sampler.env = self.envs[task_idx]
                self._start_epoch(it_)
                self.training_mode(True)
                self.collect_data(self.num_steps_prior, 1, np.inf)
                self.collect_data(self.num_steps_posterior, 1, self.update_post_train)
                num_samples_this_task += self.num_steps_prior
                num_samples_this_task += self.num_steps_posterior
                for train_step in range(self.num_train_steps_per_itr):
                    indices = np.random.choice(task_idx+1, self.meta_batch)
                    self._do_training(indices)
                    self._n_train_steps_total += 1
                gt.stamp('train')
                self.training_mode(False)
                self._try_to_eval(self._n_train_steps_total,task_idx)
                gt.stamp('eval')
                # for i in range(len(self.envs)):
                #     print(i,self.replay_buffer.num_steps_can_sample(i))
                # print(self.task_idx_train,self.task_idx,task_idx)
                self._end_epoch()
                it_+=1
    ...
    def _try_to_eval(self, epoch,index):
        logger.save_extra_data(self.get_extra_data_to_save(epoch))
    ...
    def _can_evaluate(self,epoch):
        ...
        """
        # eval collects its own context, so can eval any time
        return epoch % 5000 ==0 or epoch == 1000
    ...
    def collect_paths(self, idx, epoch, run):
        self.task_idx = idx
        self.sampler.env = self.envs[idx]
        self.envs[idx].reset()
        # print(self.sampler.env)
        ...
        while num_transitions < self.num_steps_per_eval:
            ...
            num_transitions += num
            num_trajs += 1
            # print(num_trajs,self.agent.z_means.cpu().data.numpy()[0])
            if num_trajs >= self.num_exp_traj_eval:
                self.agent.infer_posterior(self.agent.context)

        if hasattr(self.envs[0], 'tasks_pool'):
            p = paths[-1]
            done = np.sum(e['success'] for e in p['env_infos'])
            done = 1 if done >0 else 0
            p['done'] = done
    ...
    def _do_eval(self, indices, epoch):
        ...
        for idx in indices:
            ...
            for r in range(self.num_evals):
                paths = self.collect_paths(idx, epoch, r)
                all_rets.append([eval_util.get_average_returns([p]) for p in paths])
                success_single = success_single + paths[-1]['env_infos'][-1]['success']
            final_returns_last.append(np.mean([a[-1] for a in all_rets]))
            final_returns.append(np.mean([np.mean(a) for a in all_rets]))
            success_cnt.append(success_single/self.num_evals)
            # record online returns for the first n trajectories
        ...
        online_returns = [t[:n] for t in online_returns]
        #print(success_cnt, len(success_cnt))
        # print(self.agent.z_means.shape,self.agent.z_means,self.agent.z_vars)
        return final_returns[0], online_returns,final_returns_last[0], success_cnt[0],self.agent.z_means.cpu().data.numpy()[0],self.agent.z_vars.cpu().data.numpy()[0]
    ...
    def evaluate(self, epoch):
        ...
        ### sample trajectories from prior for debugging / visualization
        for i in range(len(self.envs)):
            final_returns, online_returns, final_returns_last, success_cnt,z_mean,z_var = self._do_eval(
                i, epoch)
            self.eval_statistics['AverageReturn_task_%d'%i] = final_returns
            self.eval_statistics['AverageReturn_task_last_%d'%i] = final_returns_last
            self.eval_statistics['AverageSuccess_task_last_%d'%i] = success_cnt
            self.eval_statistics['z_mean_task_%d' % i] = z_mean
            self.eval_statistics['z_var_task_last_%d' % i] = z_var
...
class MetaRLAlgorithmCont2(metaclass=abc.ABCMeta):
    def __init__(
            self,
            envs,
            agent,
    ...
    def train(self):
        ...
        for _ in range(50000):
            ...
            for idx in range(len(self.envs)):
                ...
                self.collect_data(self.num_steps_posterior, 1, self.update_post_train)
            for train_step in range(self.num_train_steps_per_itr):
                indices = np.random.choice(len(self.envs), self.meta_batch)
                self._do_training(indices)
                self._n_train_steps_total += 1
            gt.stamp('train')
            self.training_mode(False)
